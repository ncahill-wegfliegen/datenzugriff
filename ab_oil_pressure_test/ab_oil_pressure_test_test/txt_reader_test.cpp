...
public:
   ...
   TEST_METHOD( find_by_uwi )
   {
      ...
      bool actual{ reader.find( tests, uwi ) };
      Assert::IsTrue( actual, L"Failed to find the records." );
      Assert::AreEqual( static_cast<size_t>(3), tests.size(), L"Failed to find all of the tests." );
   }
   ...
   TEST_METHOD( find_by_field )
   {
      ...
      Test_container tests;
      bool actual{ reader.find( tests, field_code, pool_code ) };
      Assert::IsTrue( actual, L"Failed to find the records." );
      Assert::AreEqual( static_cast<size_t>(95), tests.size(), L"Failed to find all of the tests." );
   }
